...
function timestamp (req,res,next){
  let reqTime = new Date().toDateString();
...
app.get('/api/v1/category', timestamp , (req , res , next) =>{
  let count = dbCategory.length;
  let results = dbCategory;
  res.json({ count , results});
});

app.get('/api/v1/product',timestamp , (req , res , next) =>{
...
app.get( '/api/v1/category/:id',timestamp , (req , res , next) =>{
  let id = req.params.id;
  let record = dbCategory.filter((record) => record.id === parseInt(id));
  res.json(record);
});

app.get( '/api/v1/product/:id' ,timestamp, (req , res , next) =>{
  let id = req.params.id;
...
app.post('/api/v1/category',timestamp , (req , res , next) =>{
  let { name} = req.body;
  let record = { name} ;
...
app.put('/api/v1/category/:id',timestamp , (req , res , next) =>{
...
app.put('/api/v1/product/:id' ,timestamp, (req , res , next) =>{
  let idToUpdate = req.params.id;
...
app.delete('/api/v11/category/:id',timestamp , (req , res , next) =>{
  let id = req.params.id;
  dbCategory = dbCategory.filter((record) => record.id !== parseInt(id));
  res.json({ massage : ' Category Deleted'});
});

app.delete('/api/v11/product/:id',timestamp , (req , res , next) =>{
  let id = req.params.id;
